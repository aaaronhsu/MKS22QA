...
def menuWindow(previousWindow = None):
	...
	pricesButton = Button(menu, text = 'Changes in Cost of Items', command = lambda:pricesWindow(menu), bg = 'purple1')
	pollutionButton = Button(menu, text = 'Changes in Pollution Levels', command = lambda:pollutionWindow(menu), bg='purple1')
	deathsButton = Button(menu, text = 'Death Count', command = lambda:deathWindow(menu), bg='purple1')
...
def pricesWindow(previousWindow):
	...
	pricesSelection = Tk()
	previousWindow.withdraw()
	pricesSelection.configure(bg='yellow')
	...
	# checkboxes
	Checkbutton(pricesSelection, text = 'Toilet Paper (Small Pack, 12)', variable = toiletPaperS, onvalue = 1, offvalue = 0, bg='yellow').pack()
	Checkbutton(pricesSelection, text = 'Toilet Paper (Bulk, 24)', variable = toiletPaperL, onvalue = 1, offvalue = 0, bg='yellow').pack()
	Checkbutton(pricesSelection, text = 'Hand Sanitizer (travel size, 2 fl. oz.)', variable = handSanitizerS, onvalue = 1, offvalue = 0, bg='yellow').pack()
	Checkbutton(pricesSelection, text = 'Hand Sanitizer (medium size, 12 fl. oz.)', variable = handSanitizerL, onvalue = 1, offvalue = 0, bg='yellow').pack()
	Checkbutton(pricesSelection, text = 'Poland Spring Water Jug (1 Gallon)', variable = waterGal, onvalue = 1, offvalue = 0, bg='yellow').pack()
	Checkbutton(pricesSelection, text = 'Poland Spring Pack (24 Bottles)', variable = waterBot, onvalue = 1, offvalue = 0, bg='yellow').pack()
	Checkbutton(pricesSelection, text = 'Heavy Duty Wipes (30 ct)', variable = wipesS, onvalue = 1, offvalue = 0, bg='yellow').pack()
	Checkbutton(pricesSelection, text = 'Heavy Duty Wipes (60 ct)', variable = wipesL, onvalue = 1, offvalue = 0, bg='yellow').pack()

	# button that graphs chosen objects
	graphPrices = Button(pricesSelection, text = 'Graph Selected', bg='purple1', command = lambda:pricesData([toiletPaperS, toiletPaperL, handSanitizerS, handSanitizerL, waterGal, waterBot, wipesS, wipesL]))
	graphPrices.pack()

	# button that graphs all
	graphAll = Button(pricesSelection, text = 'Graph All', bg='purple1', command = lambda:pricesData())
	graphAll.pack()

	# return to menu button
	menuButton = Button(pricesSelection, text = 'Return to Menu', command = lambda:menuWindow(pricesSelection), bg='purple1')
	menuButton.pack()
...
def pollutionWindow(previousWindow):
	...
	pollutionSelection.configure(bg='yellow')
	pollutionSelection.title('Pollution Levels')
	label = Label(pollutionSelection, text='Choose an option:',bd = 1,relief = 'solid',width = 15,height = 2)
	label.pack()

	# Graph two countries
	graphSpecific = Button(pollutionSelection, text = 'Choose certain countries to compare', command = lambda:compareWindow(pollutionSelection), bg='purple1')
	graphSpecific.place(x=50, y=60)
	
	# Graph all
	graphAll = Button(pollutionSelection, text = 'Graph all', command = lambda:pollutionData(), bg='purple1')
	graphAll.place(x=120, y=95)
	# return to menu button
	menuButton = Button(pollutionSelection, text = 'Return to Menu', command = lambda:menuWindow(pollutionSelection), bg='purple1')
...
def compareWindow(previousWindow):
	...
	Checkbutton(countrySelection, text = 'China', variable = china, onvalue = 1, offvalue = 0, bg='yellow').pack()
	Checkbutton(countrySelection, text = 'United States', variable = unitedStates, onvalue = 1, offvalue = 0, bg='yellow').pack()
	Checkbutton(countrySelection, text = 'India', variable = india, onvalue = 1, offvalue = 0, bg='yellow').pack()
	Checkbutton(countrySelection, text = 'Global', variable = glob, onvalue = 1, offvalue = 0, bg='yellow').pack()

	# button that creates graph
	graphPrices = Button(countrySelection, text = 'Graph', command = lambda:pollutionData([china, unitedStates, india, glob]), bg='purple1')
	graphPrices.pack()

	# return to menu button
	menuButton = Button(countrySelection, text = 'Return to Menu', command = lambda:menuWindow(countrySelection), bg='purple1')
	menuButton.pack()
